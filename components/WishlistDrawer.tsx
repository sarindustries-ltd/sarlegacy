      ...
      <div className={`fixed inset-y-0 right-0 w-full sm:max-w-md h-[100dvh] bg-nexus-950/95 backdrop-blur-2xl border-l border-white/10 shadow-2xl z-[70] transform transition-transform duration-500 cubic-bezier(0.16, 1, 0.3, 1) ${isOpen ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}>
          ...
          <div className="flex items-center justify-between p-6 border-b border-white/10 bg-white/5 flex-shrink-0">
            <div className="flex items-center gap-3">
                <h2 className="text-xl font-display font-bold text-white tracking-wide">WISHLIST</h2>
                <span className="px-2 py-0.5 rounded-full bg-pink-500/20 text-pink-500 text-xs font-bold border border-pink-500/20">
                  {wishlistItems.length}
                </span>
            </div>
            <button onClick={onClose} className="p-2 text-slate-400 hover:text-white hover:bg-white/10 rounded-full transition-colors active:scale-90">
              <XIcon size={24} />
            </button>
          </div>
            ...
            {wishlistItems.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center space-y-6 animate-in fade-in zoom-in-95 duration-500">
                ...
                <div>
                  ...
                  <p className="text-slate-500 max-w-[200px] mx-auto text-sm">Save items for later tracking and analysis.</p>
                </div>
              ...
              wishlistItems.map(item => (
                <div key={item.id} className="group flex gap-4 p-4 rounded-2xl bg-white/5 border border-transparent hover:border-white/10 transition-all hover:shadow-lg relative overflow-hidden">
                  {/* Image */}
